...
Процедура ЗаполнитьНаборТестов(НаборТестов) Экспорт
	...
	НаборТестов.Добавить("ЗакрытиеЗаказаОС");
...
Процедура РасширениеТипов() Экспорт
	...
	Результат = Метаданные.Документы.ПоступлениеУслугПрочихАктивов.Движения.Содержит(Метаданные.РегистрыНакопления.ДвижениеТоваров);
	Утверждения.ПроверитьРавенство(Результат, Истина, "РегистрыНакопления.ДвижениеТоваров");
...
Процедура ЗакрытиеЗаказаОС() Экспорт
	
	СтруктураДанныхСсылки = ГенераторТестовыхДанных.СоздатьДанныеПоМакетам(ЭтотОбъект, "Организация");
	РаботаСДокументами.УдалитьДокументыПоОрганизации(СтруктураДанныхСсылки.Организация);
	
	СтруктураДанных = ГенераторТестовыхДанных.СоздатьДанныеПоМакетам(ЭтотОбъект, "ЗакрытиеЗаказа");
	
	РаботаСДокументами.ПровестиДокумент(СтруктураДанных.ЗаказПоставщику1);
	РаботаСДокументами.ПровестиДокумент(СтруктураДанных.СписаниеДС1);
	РаботаСДокументами.ПровестиДокумент(СтруктураДанных.ПоступлениеУслугИПрочихАктивов1);
	
	Запрос = Новый Запрос;
	Запрос.Параметры.Вставить("Заказ", СтруктураДанных.ЗаказПоставщику1);
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Рег.Состояние
	|ИЗ
	|	РегистрСведений.СостоянияЗаказовПоставщикам КАК Рег
	|ГДЕ
	|	Рег.Заказ = &Заказ";
	
	Состояние = Запрос.Выполнить().Выгрузить()[0][0];
	
	Утверждения.ПроверитьРавенство(Состояние, Перечисления.СостоянияЗаказовПоставщикам.Закрыт, "Состояние");
